    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (blob.CurrentShape == BlobBehavior.Shapes.Square)
        {
            if (collision.gameObject.tag == "Rhombus Weak Point")
            {
                rb2d.velocity = Vector2.zero;
                rb2d.AddForce(Jump);
                AudioSource.PlayClipAtPoint(RhombusKill, transform.position, 3f);
            }
        }
        ...
        if (blob.CurrentShape == BlobBehavior.Shapes.Square)
        {
            if (collision.gameObject.tag == "KingCubeWeakPoint")
            {
                rb2d.velocity = Vector2.zero;
                rb2d.AddForce(Jump);
                AudioSource.PlayClipAtPoint(KingCubeHurt, transform.position, 3f);
                KingCube.TakeDamage();
            }
        }
    }
